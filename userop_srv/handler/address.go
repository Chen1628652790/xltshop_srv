...
func (*UserOpServer) GetAddressList(ctx context.Context, req *proto.AddressRequest) (*proto.AddressListResponse, error) {
	...
	if result := global.MySQLConn.Where(&model.Address{User: req.UserId}).Find(&addresses); result.RowsAffected != 0 {
...
func (*UserOpServer) DeleteAddress(ctx context.Context, req *proto.AddressRequest) (*emptypb.Empty, error) {
	if result := global.MySQLConn.Where("id=? and user=?", req.Id, req.UserId).Delete(&model.Address{}); result.RowsAffected == 0 {
...
func (*UserOpServer) UpdateAddress(ctx context.Context, req *proto.AddressRequest) (*emptypb.Empty, error) {
	...
	if result := global.MySQLConn.Where("id=? and user=?", req.Id, req.UserId).First(&address); result.RowsAffected == 0 {
